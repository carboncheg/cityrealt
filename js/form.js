$(function(){
	...
	//Инициализирует красивые подсказки на полях ввода
	$('#name, #phone, #email, #message').poshytip({
	...
	//Добавляет маску вводу на поле с телефоном
	...
	//Переменные правильности заполнения
	var	name_ok = false,
		phone_ok = false,
		email_ok = false,
		message_ok = false;
	...
	//Убираем подсветку с ошибкой при фокусе на поля
	$('#name, #phone, #email, #message').focus(function(){
		$('#name, #phone, #email, #message').poshytip('hide');
		$('#name, #phone, #email, #message').removeClass('error');
	});
	...
	$submit.click(function(){
		...
		//Отправка формы, если всё заполнено верно
		//=========================================
		if(
			name_ok == true &&
			phone_ok == true &&
